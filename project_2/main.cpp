...
int distance(const std::vector<std::vector<int>>& state, const std::vector<std::vector<int>>& goal) {
    ...
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
            int value = state[i][j];
            if (value != 0) {
                int goal_i = (value - 1) / n;  // row
                int goal_j = (value - 1) % n;  // column
...
std::vector<std::vector<int>> tile_weights = {
    {1, 2, 3},
    {4, 5, 6},
    {7, 8, 9}
};
...
int orderly(const std::vector<std::vector<int>>& state, const std::vector<std::vector<int>>& goal) {
    int away = 0;
    int n = state.size();
    ...
    for (int i = 0; i < n; ++i)
    {
        ...
        {
            int value = state[i][j];
            int addative = weights[i][j];
            if (value != 0)
            {
                int goal_i = (value - 1) / n;
                int goal_j = (value - 1) % n;
                
                away += addative*(abs(i - goal_i) + abs(j - goal_j));
            }
        }
    }
    return away;
...
double euc_distance(const std::vector<std::vector<int>>& state, const std::vector<std::vector<int>>& goal) {
    double distance = 0;
    int n = state.size();
    for (int i = 0; i < n; ++i) {
        for (int j = 0; j < n; ++j) {
...
PuzzleNode* create_puzzle_node(const std::vector<std::vector<int>>& state, const std::vector<std::vector<int>>& goal, int heuristic_choice) {
    ...
    if (heuristic_choice == 1) {
        ...
        node->h = orderly(state, goal);
    } else if (heuristic_choice == 4) {
        node->h = euc_distance(state, goal);
    }else {
        std::cout << "Invalid choice. Defaulting to distance." << std::endl;
        node->h = distance(state, goal);
    }
...
void propagate_cost_downward(PuzzleNode* node, int g_propagated, const std::vector<std::vector<int>>& goal, int heuristic_choice) {
    if (g_propagated + node->h < node->f) {
        node->g = g_propagated;
        node->f = g_propagated + node->h;
        
        for (const auto& successor_state : generate_successors(node->state)) {
            PuzzleNode* successor = create_puzzle_node(successor_state, goal, heuristic_choice);
            if (node->parent != nullptr && node->parent->state == successor->state) {
                continue;
            }
            propagate_cost_downward(successor, g_propagated + 1, goal, heuristic_choice);
            delete successor;
        }
    }
}
...
std::vector<PuzzleNode*> a_star(PuzzleNode* initial_node, const std::vector<std::vector<int>>& goal, int heuristic_choice) {
    ...
    //Stores nodes in the CLOSED list
    ...
    while (!open.empty()) {
        ...
        nodes_expanded++;
        open.pop();
        //CHECKS TO SEE IF BESTNODE IS GOAL NODE
        //checks current state is the goal state
        ...
        for (const auto& successor_state : successors) {
            ...
            if (closed.find(successor_state) != closed.end()) {
                ...
                if (it != closed.end()) {
                    propagate_cost_downward(successor_node, best_node->g + 1, goal, heuristic_choice);
                }
            } else {
...
int get_total_path(const std::vector<PuzzleNode*>& path) {
    return path.size();
}
...
int main() {
    ...
    std::cout << "3. Orderly" << std::endl;
    std::cout << "4. Euclidean distance" << std::endl;
    std::cout << "Enter: ";
    std::cin >> heuristic_choice;
    std::cout << std::endl;
    
    if(heuristic_choice != 1 && heuristic_choice != 2 && heuristic_choice != 3 && heuristic_choice != 4) {
    ...
    if (!solution_path.empty()) {
        ...
        std::cout << "Total cost: " << get_total_path(solution_path) << std::endl;
    } else {
